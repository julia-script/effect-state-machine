...
declare const RuntimeExecutionIdTypeId: unique symbol
...
// Persistence brands are compile-time-only, so every public constructor erases through this seam.
...
export type RuntimeExecutionId = string & {
  readonly [RuntimeExecutionIdTypeId]: typeof RuntimeExecutionIdTypeId
}
 ...
 * @since 0.2.0
 */
export const executionId = (value: string): RuntimeExecutionId =>
  brand<string, RuntimeExecutionId>(value)
 ...
 * @since 0.2.0
 */
export const deriveRuntimeExecutionId = (
  instance: InstanceId,
  entry: EntryId,
  ownerPath: string,
  invocation: string,
 ...
 * @since 0.2.0
 */
export const MachineDefectSummary = Schema.Struct({
 ...
 * @since 0.2.0
 */
export type MachineDefectSummary = Schema.Schema.Type<typeof MachineDefectSummary>
...
export const Checkpoint = Schema.Struct({
  ...
  defect: Schema.NullOr(MachineDefectSummary),
})
...
export const ActivityOutcome = Schema.TaggedUnion({
  ...
  Defect: { defect: MachineDefectSummary },
})
...
/**
 * Persisted status and resulting revision of one idempotent external event dispatch.
 *
 * @category models
 * @since 0.2.0
 */
 ...
 * @since 0.2.0
 */
export class MachineEncodingError extends Data.TaggedError("MachineEncodingError")<{
 ...
 * **Details**
 *
 * Switch on `_tag` to distinguish definition, persistence-version, and migration-path mismatches
 ...
 * **Gotchas**
 *
 * Branch on `reason._tag`, then read the fields specific to that mismatch dimension.
 *
 * @category errors
 * @since 0.2.0
 */
 ...
 * @since 0.2.0
 */
export class MachineInstanceDefect extends Data.TaggedError("MachineInstanceDefect")<{
  readonly instanceId: InstanceId
  readonly defect: MachineDefectSummary
...
const SelectedBranch = Schema.Union([
  Schema.Struct({
    kind: Schema.Literal("guard"),
    index: Schema.Number,
    name: Schema.String,
  }),
  Schema.Struct({
    kind: Schema.Literal("otherwise"),
    index: Schema.Number,
  }),
])

const InspectionEvent = Schema.Union([
  ...
  Schema.TaggedStruct("ChildCancelled", {
    machineId: Schema.String,
    stateTag: Schema.String,
    invocation: Schema.String,
    instanceId: Schema.String,
  ...
  Schema.TaggedStruct("ChildDefected", {
    machineId: Schema.String,
    stateTag: Schema.String,
    invocation: Schema.String,
    instanceId: Schema.String,
  ...
  Schema.TaggedStruct("MachineDefected", {
    ...
    stateTag: Schema.String,
    eventTag: Schema.String,
    defect: Schema.Literal("ProtocolDefect"),
  }),
  ...
  ...(["TimerStarted", "TimerFired", "TimerCancelled"] as const).map((tag) =>
...
/** Schema-validated actor-tree body persisted by a root aggregate. */
export const PersistedTreeRecordBody = Schema.Union([
  ...
  Schema.TaggedStruct("Inspection", {
    metadata: InspectionEvent,
    event: Schema.optionalKey(Schema.Json),
  }),
  Schema.TaggedStruct("StateSnapshot", { state: Schema.Json }),
...
/** Schema-validated actor-tree body persisted by a root aggregate. */
export type PersistedTreeRecordBody = Schema.Schema.Type<typeof PersistedTreeRecordBody>

/** One globally ordered actor-tree record retained by the root aggregate. */
export const PersistedTreeRecord = Schema.Struct({
  key: Schema.String,
  sequence: Schema.Number,
  actorId: Schema.String,
  definitionPath: Schema.String,
  body: PersistedTreeRecordBody,
})

/** One globally ordered actor-tree record retained by the root aggregate. */
export type PersistedTreeRecord = Schema.Schema.Type<typeof PersistedTreeRecord>

/** Actor-tree record before the root aggregate assigns its global sequence. */
export interface TreeRecordDraft {
  readonly key: string
  readonly actorId: string
  readonly definitionPath: string
  readonly body: PersistedTreeRecordBody
}
...
export interface CreateRequest {
  ...
  readonly treeRecords?: ReadonlyArray<TreeRecordDraft>
}
...
export interface MachineCommit {
  ...
  readonly cancelExecutionIds: ReadonlyArray<RuntimeExecutionId>
...
export interface StoreService {
  ...
  /** Loads the retained root actor-tree journal, including records produced by descendants. */
  readonly loadTree: (
    instanceId: InstanceId,
  ) => Effect.Effect<ReadonlyArray<PersistedTreeRecord>, StoreError>
  ...
  /** Checks whether an activity claim still owns its delivery at the current store time. */
  readonly activityClaimActive: (
    deliveryId: DeliveryId,
    fence: number,
  ) => Effect.Effect<boolean, StoreError>

  /** Appends activity inspection facts only while the supplied claim fence remains authoritative. */
  readonly appendActivityTree: (
    deliveryId: DeliveryId,
    fence: number,
    records: ReadonlyArray<TreeRecordDraft>,
  ) => Effect.Effect<void, StoreError | LeaseLost>
...
export class Store extends Context.Service<Store, StoreService>()(
  "effect-state-machine/internal/MachineRuntimeStore",
) {}
...
export interface Handle<State, Event, Completion> {
  ...
  readonly snapshot: Effect.Effect<State, MachineError>

  /** Emits locally observed committed states until a final state is reached. */
  readonly changes: Stream.Stream<State, MachineError>

  /** Durably offers an event and waits for its keyed dispatch to commit or reject. */
  readonly send: (event: Event, options: SendOptions) => Effect.Effect<void, MachineError>

  /** Checks whether the latest committed state has a matching transition for an event. */
  readonly can: (event: Event) => Effect.Effect<boolean, MachineError>

  /** Awaits the final-state value, including a final value loaded during resume. */
  readonly completion: Effect.Effect<Completion, MachineError>

  /** Loads the latest durable lifecycle status. */
  readonly status: Effect.Effect<"running" | "completed" | "defected", MachineError>
}
