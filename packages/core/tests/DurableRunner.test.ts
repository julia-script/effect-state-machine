...
describe("Durable runner", () => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        const options: Durable.RunOptions = {
          instanceId: Durable.instanceId("timer-1"),
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            const first = yield* Durable.run(definition, { count: 1 }, options).pipe(
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            const resumed = yield* Durable.run(definition, { count: 999 }, options).pipe(
              Effect.provideService(Durable.Store, store),
            )
  ...
  it.effect("migrates the persisted document while preserving the original timer deadline", () =>
    Effect.gen(function* () {
      ...
      const instance = Durable.instanceId("migrated-timer")
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          const resumed = yield* Durable.run(
            ...
            {
              instanceId: instance,
              persistenceVersion: Durable.persistenceVersion("2"),
              migrations: [
                {
  ...
  it.effect("processes an overdue timer on resume and resets a self-reentry deadline", () =>
    Effect.gen(function* () {
      ...
      const instance = Durable.instanceId("timer-reentry")
      const options: Durable.RunOptions = {
        instanceId: instance,
        persistenceVersion: Durable.persistenceVersion("1"),
        pollIntervalMillis: 1_000,
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          const handle = yield* Durable.run(definition, { count: 0 }, options).pipe(
            Effect.provideService(Durable.Store, store),
          )
          yield* TestClock.adjust("30 seconds")
  ...
  it.effect("rejects a version mismatch without a migration path", () =>
    Effect.gen(function* () {
      ...
      const instance = Durable.instanceId("missing-migration")
      yield* Effect.scoped(
        Durable.run(
          definition,
          { count: 1 },
  ...
  it.effect("rejects a migration whose state does not decode under the current Schema", () =>
    Effect.gen(function* () {
      const store = yield* Durable.makeMemoryStore()
      const definition = makeTimerDefinition([])
      const instance = Durable.instanceId("invalid-migration")
      yield* Effect.scoped(
        Durable.run(
          definition,
          { count: 1 },
  ...
  it.effect("migrates renamed state and activity identities before workers resume", () =>
    Effect.gen(function* () {
      ...
      const v1 = v1Machine.define(
        { id: "renamed-definition", initial: ({ value }) => ({ _tag: "OldWorking", value }) },
        {
      ...
      const store = yield* Durable.makeMemoryStore()
      const instance = Durable.instanceId("renamed-instance")
      yield* Effect.scoped(
        Effect.gen(function* () {
          yield* Durable.run(
            v1,
            ...
            migrations: [
              {
                ...
                to: Durable.persistenceVersion("2"),
                migrate: (document) => {
                  const targetRevision = Durable.revision(document.checkpoint.revision + 1)
                  const nextEntry = Durable.deriveEntryId(instance, targetRevision, "NewWorking")
                  const nextKey = Durable.deriveExecutionKey(
                    instance,
                    nextEntry,
                    "NewWorking",
                    "new-work",
    ...
    () =>
      Effect.gen(function* () {
        ...
        const definition = activityMachine.define(
          { id: "durable-activity", initial: (input) => ({ _tag: "Working", value: input.value }) },
          {
            Working: activityMachine.invoke({
              ...
              effect: (state, metadata) =>
                Ref.update(executions, (keys) => [
                  ...keys,
                  metadata?.executionKey ?? "missing",
                ]).pipe(
                  Effect.as({ value: state.value * 2, key: metadata?.executionKey ?? "missing" }),
                ),
              onSuccess: {
                target: "Done",
        ...
        const options: Durable.RunOptions = {
          instanceId: Durable.instanceId("activity-1"),
          persistenceVersion: Durable.persistenceVersion("1"),
        }
        const store = yield* Durable.makeMemoryStore()
        ...
        const completed = yield* Effect.scoped(
          Effect.gen(function* () {
            const handle = yield* Durable.run(definition, { value: 21 }, options).pipe(
              Effect.provideService(Durable.Store, store),
            )
        ...
        const resumed = yield* Effect.scoped(
          Durable.run(definition, { value: 0 }, options).pipe(
            Effect.provideService(Durable.Store, store),
            Effect.flatMap((handle) => handle.completion),
          ),
        )
  ...
  it.effect("reruns an uncommitted activity after lease expiry with the same execution key", () =>
    Effect.gen(function* () {
      ...
      const definition = machine.define(
        { id: "durable-redelivery", initial: () => ({ _tag: "Working" }) },
        {
          Working: machine.invoke({
            ...
            effect: (_, metadata) =>
              Effect.gen(function* () {
                const keys = yield* Ref.updateAndGet(executions, (current) => [
                  ...current,
                  metadata?.executionKey ?? "missing",
                ])
                if (keys.length === 1) return yield* Effect.never
                return metadata?.executionKey ?? "missing"
              }),
      ...
      const store = yield* Durable.makeMemoryStore()
      const options: Durable.RunOptions = {
        instanceId: Durable.instanceId("redelivery"),
        persistenceVersion: Durable.persistenceVersion("1"),
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          yield* Durable.run(definition, {}, options).pipe(
            Effect.provideService(Durable.Store, store),
          )
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          assert.deepStrictEqual(keys, [done.key, done.key])
        }),
      )
    }),
  )
  ...
  it.effect("persists an activity defect summary while retaining its live Cause", () =>
    Effect.gen(function* () {
      ...
      const definition = machine.define(
        { id: "durable-defect", initial: () => ({ _tag: "Working" }) },
        {
          Working: machine.invoke({
            name: "explode",
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          const handle = yield* Durable.run(
            definition,
            {},
            {
  ...
  it.effect("checkpoints all lanes and completes a parallel-region macrostep", () =>
    Effect.gen(function* () {
      ...
      const machine = Machine.builder({ input: Input, state: State, event: Event })
      const definition = machine.define(
        { id: "durable-aggregate-regions", initial: () => ({ _tag: "Loading" }) },
        {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          const handle = yield* Durable.run(
            ...
            {},
            {
              instanceId: Durable.instanceId("aggregate-regions"),
              persistenceVersion: Durable.persistenceVersion("1"),
            },
          ).pipe(Effect.provideService(Durable.Store, store))
  ...
  it.effect("preserves race winner and value correlation through the persisted outcome", () =>
    Effect.gen(function* () {
      ...
      const machine = Machine.builder({ input: Schema.Struct({}), state: State, event: Event })
      const definition = machine.define(
        { id: "durable-race", initial: () => ({ _tag: "Racing" }) },
        {
      ...
      const completed = yield* Effect.scoped(
        Durable.run(
          ...
          {},
          {
            instanceId: Durable.instanceId("race"),
            persistenceVersion: Durable.persistenceVersion("1"),
  ...
  it.effect("resumes bounded all work from persisted partial lane progress", () =>
    Effect.gen(function* () {
      ...
      const definition = machine.define(
        { id: "durable-partial-all", initial: () => ({ _tag: "Loading" }) },
        {
          Loading: machine.invoke.all({
            name: "bounded",
            concurrency: 1,
      ...
      const store = yield* Durable.makeMemoryStore()
      const options: Durable.RunOptions = {
        instanceId: Durable.instanceId("partial-all"),
        persistenceVersion: Durable.persistenceVersion("1"),
        activityLeaseMillis: 100,
        pollIntervalMillis: 25,
      }
    ...
    Effect.gen(function* () {
      ...
      const completed = yield* Effect.scoped(
        Durable.run(
          ...
          { base: 40 },
          {
            instanceId: Durable.instanceId("region-work"),
            persistenceVersion: Durable.persistenceVersion("1"),
          },
        ).pipe(
          Effect.provideService(Durable.Store, store),
