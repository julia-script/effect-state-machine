...
const checkpoint = (
  ...
  overrides: Partial<Runtime.Checkpoint>,
): Runtime.Checkpoint => ({
  formatVersion: 1,
...
const resumeError = (store: Runtime.StoreService, id: Runtime.InstanceId) =>
  Effect.flip(
    Effect.scoped(
      Runtime.run(
        definition,
...
describe("Runtime persistence boundaries", () => {
  it.effect("rejects an active region omitted by the encoded parent state", () =>
    Effect.gen(function* () {
      ...
      const id = Runtime.instanceId("missing-encoded-region-slot")
  ...
  it.effect("publishes a region activity from the encoded slot representation", () =>
    Effect.gen(function* () {
      ...
      const store = yield* Runtime.makeMemoryStore()
      const id = Runtime.instanceId("encoded-region-slot")
      yield* Effect.scoped(
        Effect.asVoid(
          Runtime.run(regionDefinition, "hello", {
            instanceId: id,
            persistenceVersion: Runtime.persistenceVersion("1"),
          }).pipe(Effect.provideService(Runtime.Store, store)),
        ),
  ...
  it.effect("distinguishes definition identity incompatibility", () =>
    Effect.gen(function* () {
      const store = yield* Runtime.makeMemoryStore()
      const id = Runtime.instanceId("definition-mismatch")
      yield* store.create({
  ...
  it("keeps boundary causes live without placing them in persisted defect summaries", () => {
    ...
    const storeError = new Runtime.StoreError({
      operation: "load",
      message: cause.message,
      cause,
    })
    const encodingError = new Runtime.MachineEncodingError({
      operation: "encode",
      message: cause.message,
      cause,
    })
    const migrationError = new Runtime.MigrationError({
      instanceId: Runtime.instanceId("cause"),
      message: cause.message,
      cause,
    })
    ...
    assert.strictEqual("cause" in Runtime.MachineDefectSummary.fields, false)
  })

  it.effect("rejects malformed values under every published durable envelope Schema", () =>
    Effect.gen(function* () {
      const id = Runtime.instanceId("malformed-envelope")
      const validCheckpoint = checkpoint(id, {})
      const validations: ReadonlyArray<Effect.Effect<void, Schema.SchemaError>> = [
        Effect.asVoid(
          Schema.decodeUnknownEffect(Schema.toCodecJson(Runtime.Checkpoint))({
            ...validCheckpoint,
            state: undefined,
          }),
        ),
        Effect.asVoid(
          Schema.decodeUnknownEffect(Schema.toCodecJson(Runtime.MachineMessage))({
            _tag: "External",
            messageId: "message",
            instanceId: id,
        ...
        Effect.asVoid(
          Schema.decodeUnknownEffect(Schema.toCodecJson(Runtime.ActivityCommand))({
            deliveryId: "activity",
            instanceId: id,
        ...
        Effect.asVoid(
          Schema.decodeUnknownEffect(Schema.toCodecJson(Runtime.ActivityOutcome))({
            _tag: "Success",
            encodedValue: undefined,
          }),
        ),
        Effect.asVoid(
          Schema.decodeUnknownEffect(Schema.toCodecJson(Runtime.DispatchRecord))({
            instanceId: id,
            idempotencyKey: "event",
            payloadFingerprint: "fingerprint",
            status: "pending",
        ...
        Effect.asVoid(
          Schema.decodeUnknownEffect(Schema.toCodecJson(Runtime.MigrationDocument))({
