...
var taggedUnion = (cases) => {
  const members = Object.entries(cases).map(([tag, definition2]) => {
    ...
    return definition2.title === void 0 && definition2.description === void 0 ? member : member.annotate({
...
var decodeInput = (definition2, input) => Schema.decodeUnknownEffect(definition2.schemas.input)(input);
var encodeInput = (definition2, input) => Schema.encodeEffect(definition2.schemas.input)(input);
var decodeState = (definition2, input) => Schema.decodeUnknownEffect(definition2.schemas.state)(input);
var encodeState = (definition2, input) => Schema.encodeEffect(definition2.schemas.state)(input);
var decodeEvent = (definition2, input) => Schema.decodeUnknownEffect(definition2.schemas.event)(input);
...
var run2 = (definition2, input) => Effect.gen(function* () {
  ...
  const inspection = (projectEvent) => SubscriptionRef.changes(inspectionRef).pipe(
    Stream.mapAccum(
      ...
      (seen, records) => [
        records.length,
        records.slice(seen).map(
          (record) => projectEvent === void 0 || record.event === void 0 ? record.metadata : { ...record.metadata, details: projectEvent(record.event) }
        )
      ]
    )
  );
...
var internalFrame = (file) => /(?:^|\/)src\/(?:Machine|Source)\.[cm]?[jt]s$/.test(file) || /(?:^|\/)dist\/(?:Machine|Source)\.js$/.test(file) || /node_modules\/effect-state-machine\/(?:src|dist)\/(?:Machine|Source)/.test(file);
var location = (file, line, column, functionName) => {
  const parsedLine = Number(line);
  const parsedColumn = Number(column);
  if (file.length === 0 || file === "<anonymous>" || file.startsWith("node:") || /^https?:/.test(file) && /\.html(?:$|[?#])/.test(file) || !Number.isInteger(parsedLine) || !Number.isInteger(parsedColumn) || parsedLine < 1 || parsedColumn < 1) {
    return void 0;
  }
  let decoded = file;
...
var activity = (graph, activeNode, transitions) => ({
  ...activeNode === void 0 || !graph.nodes.some((node) => node.id === activeNode) ? {} : { activeNode },
  traversedEdges: graph.edges.flatMap(
    (edge) => transitions.some(
      (transition) => transition.source === edge.source && transition.target === edge.target && (transition.event === void 0 || transition.event === edge.event?.tag) && (transition.branchIndex === void 0 || transition.branchIndex === edge.branch?.index)
    ) ? [edge.id] : []
  )
});
...
var fromDefinition = (definition2, options) => {
        ...
        invocation: {
          ...
          ...node.retry === void 0 ? {} : {
            retry: {
              ...
              ...node.retry.description === void 0 ? {} : { description: node.retry.description }
            }
          }
        }
      } : {},
            ...
            return [
              {
                ...
                ...forwarded.description === void 0 ? {} : { description: forwarded.description }
              }
            ];
          })
        }
    ...
    for (const [eventTag, handler] of Object.entries(node.on)) {
      ...
      if ("ignore" in handler) {
        ignores.push({
          source: node.tag,
          event,
          ...handler.ignore.description === void 0 ? {} : { description: handler.ignore.description }
        });
        continue;
      }
      if (!("branches" in handler)) {
      ...
      for (const [index, branch] of handler.branches.entries()) {
        const branchMetadata = "otherwise" in branch ? { kind: "otherwise", index } : {
          kind: "guard",
          index,
          name: branch.when.name,
          ...branch.when.description === void 0 ? {} : { description: branch.when.description }
        };
        edges.push({
          source: node.tag,
      ...
      for (const [kind, outcomeHandler] of [
        ...
        if (!("branches" in outcomeHandler)) {
          edges.push({
            ...
            outcome: { kind },
            ...outcomeHandler.description === void 0 ? {} : { description: outcomeHandler.description }
          });
          continue;
        }
        for (const [index, outcome] of outcomeHandler.branches.entries()) {
          const branch = "otherwise" in outcome ? { kind: "otherwise", index } : {
            kind: "guard",
            index,
            name: outcome.when.name,
            ...outcome.when.description === void 0 ? {} : { description: outcome.when.description }
          };
          edges.push({
            source: node.tag,
  ...
  const identifiedEdges = edges.map((edge, index) => {
    ...
    if (authoredNode !== void 0 && edge.branch?.kind === "guard") {
      const handler = edge.event !== void 0 && authoredNode.kind !== "final" ? authoredNode.on[edge.event.tag] : authoredNode.kind === "invoke" && edge.outcome?.kind === "success" ? authoredNode.onSuccess : authoredNode.kind === "invoke" && edge.outcome?.kind === "failure" ? authoredNode.onFailure : authoredNode.kind === "child" && edge.outcome?.kind === "completion" ? authoredNode.onComplete : void 0;
...
var make6 = (options) => ({
  ...
  machine: {
    ...
    ...options.definition.description === void 0 ? {} : { description: options.definition.description }
  },
...
var attach = (options) => Effect2.gen(function* () {
  ...
  const pump = (connection) => Effect2.gen(function* () {
    while (true) {
      ...
      yield* Ref2.update(
        buffer,
        (state) => state.facts[0] === fact ? { ...state, facts: state.facts.slice(1) } : state
      );
    }
  });
  const listen = (connection) => Effect2.gen(function* () {
    while (true) {
  ...
  yield* Effect2.addFinalizer(
    () => Effect2.gen(function* () {
      ...
      yield* connection.send({ _tag: "SessionEnded", sessionId }).pipe(Effect2.catch(() => Effect2.void));
    })
  );
  return { sessionId };
});
...
var make9 = (options) => ({
  connect: Effect3.gen(function* () {
    ...
    yield* socket.runString(
      (text) => decodeMessageString(text).pipe(
    ...
    const connection = {
      send: (message) => encodeMessageString(message).pipe(
...
var OrdersLive = Layer2.effect(
  ...
  Effect4.gen(function* () {
    ...
    return Orders.of({
      place: (total) => Ref3.updateAndGet(attempts, (attempt) => attempt + 1).pipe(
        Effect4.delay("900 millis"),
        Effect4.flatMap(
          (attempt) => attempt % 2 === 1 ? Effect4.fail(new PaymentDeclined({ message: "card declined" })) : Effect4.succeed(`order-${total}-${attempt}`)
        )
      )
    });
  })
