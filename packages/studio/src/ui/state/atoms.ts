...
const runtime = Atom.runtime(ViewerClient.layer())
...
const kvsRuntime = Atom.runtime(KeyValueStore.layerStorage(() => globalThis.localStorage))
...
export const sourceActionAtom = Atom.kvs({
  runtime: kvsRuntime,
  key: "studio-source-action",
  schema: Schema.Literals(["editor", "copy"]),
  defaultValue: () => "editor" as const,
})
