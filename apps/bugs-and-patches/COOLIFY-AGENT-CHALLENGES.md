...
Agent challenge URLs contain a short-lived bearer capability in their path. The application logger is disabled, but a reverse proxy with access logging enabled can still retain that path. Production must verify that proxy access logging is disabled globally or route `/challenge/` through a dedicated higher-priority router whose access logs are disabled.
...
3. Inspect the server's generated proxy configuration. Traefik access logging is disabled by default; when the proxy command has no `--accesslog` option, no additional challenge router is required. Record that check and continue to step 5.
4. If access logging is enabled, first verify that the deployed Traefik version supports per-router observability. Then inspect the application’s generated labels to learn its real HTTPS entrypoint, service name, certificate resolver, and generated router name. Do not guess them. Add a separate challenge router through the application’s **Custom Labels**, adapting the placeholders below to those generated values:
   ...
   The access-log label is the security-critical line. Keep the ordinary application router for `/auth`, `/api`, `/game`, and `/health` unchanged. Traefik 3.1 does not support this per-router option and rejects the container's dynamic routing when the label is present; on that version, either leave global access logging disabled or upgrade before enabling it.
